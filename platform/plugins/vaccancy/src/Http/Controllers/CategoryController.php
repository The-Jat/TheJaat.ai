...
use Botble\Vaccancy\Forms\CategoryForm;
use Botble\Vaccancy\Http\Requests\CategoryRequest;
...
class CategoryController extends BaseController
{
    ...
    {
        PageTitle::setTitle(trans('plugins/vaccancy::categories.menu'));
    ...
    {
        PageTitle::setTitle(trans('plugins/vaccancy::categories.create'));
    ...
    public function edit(VaccancyCategory $category, FormBuilder $formBuilder, Request $request, BaseHttpResponse $response)
    {
        if ($request->ajax()) {
    ...
    public function update(VaccancyCategory $category, CategoryRequest $request, BaseHttpResponse $response)
    {
        if ($request->input('is_default')) {
    ...
    public function destroy(VaccancyCategory $category, Request $request, BaseHttpResponse $response)
    {
        try {
    ...
    protected function getForm(?VaccancyCategory $model = null): string
    {
    ...
    protected function setFormOptions(FormAbstract $form, ?VaccancyCategory $model = null, array $options = []): FormAbstract
    {
        if (! $model) {
            $form->setUrl(route('vaccancy_categories.create'));
        }

        if (! Auth::user()->hasPermission('vaccancy_categories.create') && ! $model) {
    ...
    {
        return array_merge([
            'canCreate' => Auth::user()->hasPermission('vaccancy_categories.create'),
            'canEdit' => Auth::user()->hasPermission('vaccancy_categories.edit'),
            'canDelete' => Auth::user()->hasPermission('vaccancy_categories.destroy'),
            'createRoute' => 'vaccancy_categories.create',
            'editRoute' => 'vaccancy_categories.edit',
            'deleteRoute' => 'vaccancy_categories.destroy',
        ], $options);
    }
}
