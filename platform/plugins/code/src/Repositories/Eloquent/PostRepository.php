...
class PostRepository extends RepositoriesAbstract implements PostInterface
{
    ...
    {
        $model = $model instanceof CodePost ? $model : $this->findById($model);
...
// dd($this->model);
        $data = $this->model
            ->where('code_posts.status', BaseStatusEnum::PUBLISHED)
            ->join('post_categories', 'post_categories.post_id', '=', 'code_posts.id')
            ->join('categories', 'post_categories.category_id', '=', 'categories.id')
            ->whereIn('post_categories.category_id', $categoryId)
            ->select('code_posts.*')
            ->distinct()
            ->with('slugable')
            ->orderBy('code_posts.created_at', 'desc');
    ...
    public function getAllRootCodes(bool $active = true): Collection
    {
        ...
        $sortBy = theme_option('sort_by', 'order');
        $sortIn = theme_option('sort_order', 'asc');

        if ($active) {
            $data = $data->where('parent_id', 0)
                        ->where('status', BaseStatusEnum::PUBLISHED)
                        ->orderBy($sortBy, $sortIn);
        }
    ...
    public function getAllCodePostsByParentId(int $id, $paginate = 10): Collection|LengthAwarePaginator
    {
        $data = $this->model
        ->where('parent_id', $id);
    ...
    public function getDifficultyCodePostsByParentId(int $id, array $filters, $paginate = 10): Collection|LengthAwarePaginator
    {
        $data = $this->model
            ->where('parent_id', $id)
            ->where('status', BaseStatusEnum::PUBLISHED);
        ...
        if ($difficulty !== "all") {
            $data = $data->where('difficulty', $difficulty);
        }

        $orderBy = Arr::get($filters, 'orderBy', 'updated_at');
        $order = Arr::get($filters, 'order', 'desc');
