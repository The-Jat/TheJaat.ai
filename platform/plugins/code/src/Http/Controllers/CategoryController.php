...
use Botble\Code\Forms\CategoryForm;
use Botble\Code\Http\Requests\CategoryRequest;
...
class CategoryController extends BaseController
{
    ...
    {
        PageTitle::setTitle(trans('plugins/code::categories.menu'));
    ...
    {
        PageTitle::setTitle(trans('plugins/code::categories.create'));
    ...
    public function store(CategoryRequest $request, BaseHttpResponse $response)
    {
        ...
        return $response
            ->setPreviousUrl(route('code_categories.index'))
            ->setNextUrl(route('code_categories.edit', $category->id))
            ->setMessage(trans('core/base::notices.create_success_message'));
    }

    public function edit(CodeCategory $category, FormBuilder $formBuilder, Request $request, BaseHttpResponse $response)
    {
        if ($request->ajax()) {
    ...
    public function update(CodeCategory $category, CategoryRequest $request, BaseHttpResponse $response)
    {
        if ($request->input('is_default')) {
    ...
    public function destroy(CodeCategory $category, Request $request, BaseHttpResponse $response)
    {
        try {
    ...
    protected function getForm(?CodeCategory $model = null): string
    {
    ...
    protected function setFormOptions(FormAbstract $form, ?CodeCategory $model = null, array $options = []): FormAbstract
    {
        if (! $model) {
            $form->setUrl(route('code_categories.create'));
        }

        if (! Auth::user()->hasPermission('code_categories.create') && ! $model) {
    ...
    {
        return array_merge([
            'canCreate' => Auth::user()->hasPermission('code_categories.create'),
            'canEdit' => Auth::user()->hasPermission('code_categories.edit'),
            'canDelete' => Auth::user()->hasPermission('code_categories.destroy'),
            'createRoute' => 'code_categories.create',
            'editRoute' => 'code_categories.edit',
            'deleteRoute' => 'code_categories.destroy',
        ], $options);
    }
}
