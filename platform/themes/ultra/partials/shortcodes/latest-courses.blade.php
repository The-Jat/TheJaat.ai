    ...
    <div class="container">
        <div class="sidebar-widget">
            ...
            <div class="post-carausel-2 post-module-1 row">
                @foreach (get_latest_courses($shortcode->limit) as $post)
                    <div class="col">
                        <div class="post-thumb position-relative">
                            <div class="thumb-overlay img-hover-slide border-radius-5 position-relative lazy"
                                ...
                                <a class="img-link" href="{{ get_external_link($post) }}" {{ is_external_link_course($post) ? 'target="_blank"' : '' }}></a>
                                @if(is_external_link_course($post))
                                    <span class="top-right-icon background10"><i class="ti-link" aria-hidden="true"></i></span>
                                @endif
                                <div class="post-content-overlay">
                                    <h6 class="post-title">
                                        <a class="color-white" href="{{ get_external_link($post) }}" {{ is_external_link_course($post) ? 'target="_blank"' : '' }}>{{ $post->name }}</a>
                                    </h6>
