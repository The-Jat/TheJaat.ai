...
@php
    ...
    if (is_plugin_active('pro-posts')) {
        Theme::asset()
            ->container('footer')
            ->usePath(false)
            ->add('favorite-post', 'vendor/core/plugins/pro-posts/js/favorite-posts.js');
        Theme::asset()
            ->container('footer')
            ->usePath()
            ->add('post-js', 'js/post.js');
    }

    // dd($post->author->firstName());
    // dd(get_all_vaccancy_categories());
    $allCategories = get_all_vaccancy_categories();
    dd($allCategories);
@endphp
        ...
        <div class="row">
            <div class="col-lg-8 col-md-12 col-sm-12">
                @if ($singleLayout == 'default' || empty($singleLayout))
                    {!! Theme::partial(
                        'plugins/vaccancy::partials.vaccancy-default'
                        compact('post', 'totalComment', 'videoLink', 'videoEmbedCode', 'videoUploadId'))
                    !!}
                @endif
                ...
                @if (is_plugin_active('external-source') && !empty($post->source_link))
                    <div class="entry-bottom mt-50 mb-30">
                        <p>{{ __('Source link') }}:
                            <a href="{{ $post->source_link }}" class="blue"
                                target="_blank">{{ $post->source_link }}</a>
                        </p>
                    </div>
                @endif
                        ...
                        <div class="author-image mb-md-30">
                            <a href="{{ $post->author->url }}">
                                <img class="avatar"
                                    ...
                                    loading="lazy" {{-- alt="{{ $post->author->getFullName() }}" --}}>
                            </a>
                        </div>
                            ...
                            <a href="{{ $post->author->url }}">
                                <h4 class="font-weight-bold mb-20">
                                    <span class="vcard author">
                                        <span
                                            class="fn">{{ $post->author->first_name . ' ' . $post->author->last_name }}</span>
                                    </span>
                                </h4>
                            </a>
