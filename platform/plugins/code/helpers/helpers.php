...
if (!function_exists('get_code_parents')) {
    /**
     * @param array $args
     * @return array
     */
    function get_code_parents(array $args = []): array
    {
        $indent = Arr::get($args, 'indent', '——');

        $repo = app(PostInterface::class);

        $categories = $repo->getCodes(Arr::get($args, 'select', ['*']), [
            'created_at' => 'DESC',
            //'is_default' => 'DESC',
            //'order'      => 'ASC',
        ]);
...
if (! function_exists('get_code_categories')) {
    function get_code_categories(array $args = []): array
    {
        $indent = Arr::get($args, 'indent', '——');
...
if (! function_exists('get_categories_with_children_of_codes')) {
    function get_categories_with_children_of_codes(): array
    {
...
if (! function_exists('get_code_page_id')) {
    function get_code_page_id(): string|null
    {
        return theme_option('code_page_id', setting('code_page_id'));
    }
}

if (! function_exists('get_code_page_url')) {
    function get_code_page_url(): string
    {
        $codePageId = (int)theme_option('code_page_id', setting('code_page_id'));

        if (! $codePageId) {
            return route('public.index');
        }

        $codePage = app(PageInterface::class)->findById($codePageId);
    ...
    if (!function_exists('get_all_code_posts_by_parent_id')) {
        function get_all_code_posts_by_parent_id(int $id, int $paginate): Collection|LengthAwarePaginator
        {
            return app(PostInterface::class)->getAllCodePostsByParentId($id, $paginate);
        }
    }

    if (!function_exists('get_difficulty_code_posts_by_parent_id')) {
        function get_difficulty_code_posts_by_parent_id(int $id, array $args = [], int $paginate): Collection|LengthAwarePaginator
        {
            // $difficulty = Arr::get($args, 'difficulty', 'all');
            // $orderBy = Arr::get($args, 'orderBy', 'order');
            // $order = Arr::get($args, 'order', 'order');

            return app(PostInterface::class)->getDifficultyCodePostsByParentId($id, $args, $paginate);
        }
    }

    if (!function_exists('is_external_link_code_post')) {
        function is_external_link_code_post(CodePost $post): bool
        {
